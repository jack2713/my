...
def fetch_and_replace(urls):
    ...
    # Define a User-Agent string to simulate a browser (this example uses a common Chrome User-Agent)
    headers = {
    ...
    for url in urls:
        try:
            ...
            response = requests.get(url, headers=headers, timeout=15)  # 添加 headers 参数
            end_time = time.time()
